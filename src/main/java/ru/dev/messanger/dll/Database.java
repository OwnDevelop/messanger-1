...
//    private static final String URL = "jdbc:mariadb://localhost:3306/messenger?useUnicode=yes&characterEncoding=UTF-8";
    ...
    public NewUserDTO getUserByACode(String code) {
        ...
        try (Connection connection = DriverManager.getConnection(properties.getProperty("url"), properties)) {
            String SqlQuery = "SELECT users.id, email, login, password, first_name, last_name, sex, created_at," +
                    " status, avatar, activation_code, value as status, url as avatar " +
                    "FROM users LEFT JOIN status ON users.status=status.id LEFT JOIN photos ON users.avatar=photos.id " +
            ...
            for (int i : conversations_id) {
                ...
                while (rs.next()) {
                    ...
                    if (rs.getInt(1) > 2) {
                        ...
                        ResultSet rst = getResult(SqlQuery);
                        while (rst.next()) {
    ...
    private Integer addImage(String url) {
        Integer id = 0;
